...
function createMessage(post, board, isOP) {
    ...
    messageContainer.className = `message-container ${isOP ? 'op' : 'mine'}`;
    messageContainer.id = `post-${post.no}`;

    const message = document.createElement('div');
    message.className = `message ${isOP ? 'op' : 'mine'}`;
    ...
    const opTag = isOP ? '<span class="op-tag">OP</span>' : '';

    message.innerHTML = `
        <div class="message-header">
            ${replyLinks}
            <span class="post-no">Post #${post.no}</span>
            <span class="post-user">${userId}${opTag}</span>
